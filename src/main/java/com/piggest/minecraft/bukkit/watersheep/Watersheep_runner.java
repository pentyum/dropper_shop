...
public class Watersheep_runner extends BukkitRunnable {
	...
	public Watersheep_process_code process_a_sheep(Entity sheep) {
		...
		if(!s.isSheared()) {
			...
			Location grass_supposed_loc = new Location(sheep_loc.getWorld(), Math.round(sheep_loc.getX()), Math.round(sheep_loc.getY()) - 1, Math.round(sheep_loc.getZ()));
	...
	public void run() {
			...
			for(String one_world_name: worlds_with_players) {
				...
				for(Chunk one_loaded_chunk: loaded_chunks) {
					...
					for(Entity one_entity: entities_in_this_chunk) {
						if(one_entity.getType() != EntityType.SHEEP || !one_entity.getScoreboardTags().contains("watersheep")) {
							continue;
						}
		...
		String msg = "[植物羊]在加载的 " + counter_how_many_sheep_proceesed + " 只植物羊里面，有 " + counter_how_many_sheep_not_sheared + " 只本身有毛，有 " + counter_how_many_sheep_no_grass + " 只脚下没有草，有 " + counter_how_many_sheep_succeed + " 只成功长出了毛。";
