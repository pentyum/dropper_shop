	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (command.getName().equalsIgnoreCase("watersheep")) {
			...
			switch (subcommand) {
				case curse:
					/*
					 * 把羊变成植物羊有以下步骤： 搜索距离在16个格子以内的所有实体 检查羊是否和玩家在同一区块 使用 NMS 开启羊的 NoAI 属性
					 * 把羊加入一个计分板，名称为 watersheep 设置羊为静音状态
					 */
				{
					...
					for (Entity one_nearby_sheep : nearby_sheep) {
						if (one_nearby_sheep != null) {
							...
							/* 检查羊是否和玩家在同一区块 */
							if (player_chunk.getX() != one_nearby_sheep_chunk.getX()
									|| player_chunk.getZ() != one_nearby_sheep_chunk.getZ()) {
								continue;
							}
							...
							/* 开启羊的 NoAI 属性 */
							LivingEntity living_entity = (LivingEntity) one_nearby_sheep;
							living_entity.setAI(false);
				...
				case bless:
					/*
					 * 把植物羊变回羊有以下步骤： 搜索距离在16个格子以内的所有实体 检查羊是否和玩家在同一区块 检查羊是否有计分板项目 watersheep 使用 NMS
					 * 关闭羊的 NoAI 属性 设置羊为非静音状态
					 */
...

				{
					...
					Collection<Entity> nearby_sheep = player_world.getNearbyEntities(player_location, 128, 128, 128,
							isSheep);
					...
					player.sendMessage("[植物羊]附近的 16x16 区块中有 " + counter_how_many_normal_sheep + " 只普通羊和 "
							+ counter_how_many_watersheep + " 只植物羊");
				}
