...
public class Advanced_furnace_command_executor implements TabExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (!(sender instanceof Player)) { // 如果sender与Player类不匹配
			sender.sendMessage("[高级熔炉]必须由玩家执行该命令");
			return true;
		}
		Player player = (Player) sender;
		if (cmd.getName().equalsIgnoreCase("adv_furnace")) {
			if (args.length == 0) {
				player.sendMessage("[高级熔炉]请使用/adv_furnace remove|temp|getmoney");
				return true;
			}
			if (args[0].equalsIgnoreCase("temp")) {
				player.sendMessage("当前你所在位置的温度:"
			...
			if (args[0].equalsIgnoreCase("remove")) {
				...
				adv_furnace.remove();
				player.sendMessage("[高级熔炉]高级熔炉结构已经移除");
				return true;
				...
				if (!player.hasPermission("adv_furnace.admin")) {
					player.sendMessage("[高级熔炉]你没有权限直接设置等级");
					return true;
				}
				Advanced_furnace adv_furnace = Dropper_shop_plugin.instance.get_adv_furnace_manager()
						.find_existed(look_block.getLocation());
				...
				if (args.length < 2) {
					player.sendMessage("[高级熔炉]没有输入等级");
					return true;
				}
				int level = 0;
				try {
					...
					player.sendMessage("[高级熔炉]等级不是数字");
					return true;
				}
