...
public class Anti_thunder_listener implements Listener {
	...
	public void on_thunder(LightningStrikeEvent event) {
		...
		Chunk_location chunk_loc = new Chunk_location(event.getLightning().getLocation().getChunk());
		...
		if(find != null) {
			for(Structure structure : find) {
				...
				if (anti_thunder.completed() == false) {
					Dropper_shop_plugin.instance.getLogger().info("区块" + anti_thunder.get_chunk_location() + "的防雷器结构不完整，已经移除");
					anti_thunder.remove();
					continue;
				}
